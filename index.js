...
function sanitizeCategories(categories) {
  ...
  const clean = categories.filter(c => c && typeof c.id === 'string' && typeof c.name === 'string').slice(0, 15)
...
function activePlayerIds(room) {
  return Object.keys(room.players).filter(pid => room.players[pid].connected !== false && !room.skippedPlayers.includes(pid));
}
...
try {
  ...
  console.warn('Dictionary package missing:', e.message);
}
...
app.get('/api/validate', (req, res) => {
  ...
  res.json({
    ...
    inDictionary: dictReady ? inDictionary(word) : null, dictReady,
...
io.on('connection', (socket) => {
  ...
  socket.on('skip_player', ({ roomCode, playerId } = {}) => {
    ...
    io.to(roomCode).emit('player_skipped', {
      playerId, playerName: room.players[playerId].name, skippedPlayers: room.skippedPlayers,
    });
  ...
  socket.on('player_ready', ({ roomCode } = {}) => {
    ...
    if (room.state === 'waiting_next' && room.round >= room.maxRounds) {
      ...
      if (ids.length >= 1 && ready.length >= ids.length) {
        room.state = 'finished';
        clearRoundTimer(room);
        const finPayload = {
          totalScores: publicPlayersScores(room),
  ...
  socket.on('chat_message', ({ roomCode, message } = {}) => {
    ...
    io.to(roomCode).emit('chat_message', {
      playerName: room.players[socket.id].name, message: String(message).slice(0, 300),
    });
  });
