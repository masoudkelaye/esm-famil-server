...
io.on('connection', (socket) => {
  ...
  socket.on('next_round', ({ roomCode } = {}) => {
    ...
    if (room.round >= room.maxRounds) {
      room.state = 'finished';
      clearRoundTimer(room);
      const finPayload = { totalScores: publicPlayersScores(room), players: publicPlayers(room), lastResults: room.lastResults || {} };
      io.to(roomCode).emit('game_finished', finPayload);
      // ensure host receives even if room broadcast is flaky
  ...
  socket.on('player_ready', ({ roomCode } = {}) => {
    ...
    // if everyone ready on last round → finish for all (including host)
